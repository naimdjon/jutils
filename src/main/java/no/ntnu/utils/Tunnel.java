...
public class Tunnel {
    ...
    public static void tunnel(String remoteHost, int portNumber) {
        ...
        try {
            ...
            Debug.i("Connecting to %s", loginserver);
            ...
            if (exists(id_rsa)) {
                ...
                Debug.i("logging with password.");
                invoke(session, "setPassword", System.getProperty(pass));
            } else {
                Debug.w("~/.ssh/id_rsa did not exist! Passwordless login will probably result in Auth fail!");
            }
            ...
            Debug.d("forwarding: localhost:" + assinged_port + " -> localhost:" + portNumber);
    ...
    private static void retryWithPassword(String remoteHost, int portNumber, String loginserver, RuntimeException e) {
        if (contains(stackTraceToStr(e), ("Auth fail")) && !Boolean.getBoolean("nt-utils.tunnel.retryWithPassword")) {
            Debug.e("Authentication failed. Please enter your ssh password [%s]: ", loginserver);
    ...
    private static void checkAndSetPass() {
        if (exists(config)) {
            Debug.i("config file [%s] exists!", config.getAbsolutePath());
    ...
    static void registerCleanup() {
        runOnExit(new Runnable() {
            public void run() {
                Debug.d("cleaning up ssh sessions!");
